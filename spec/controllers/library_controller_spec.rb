...
RSpec.describe LibraryController, type: :controller do
  ...
  describe 'POST user_contact_form' do
    it 'redirect and return a success message' do
      ...
      post :user_contact_form, params: { full_name: 'John Doe', email_address: 'johndoe77@unknownemail.com', type: 'Contact Us', question: 'Where is the contact form?' }, format: :js

      expect(flash[:modal_success]).to eq('Thank you! Your submission was successful. We will contact you shortly.')
      expect(response.status).to eq(200)
    end
    ...
    it 'redirect and cancel submission with unverified recaptcha' do
      ...
      expect(flash[:modal_success]).to be_nil
      expect(flash[:modal_error]).to eq('Your submission was not successful. Please try again.')
      expect(response.status).to eq(200)
    end

    it 'redirect and cancel submission with missing param' do
      post :user_contact_form, params: { full_name: 'John Doe', email_address: 'johndoe77@unknownemail.com', type: 'Contact Us' }, format: :js
    ...
    it 'redirect and cancel submission with empty param' do
      post :user_contact_form, params: { full_name: 'John Doe', email_address: 'johndoe77@unknownemail.com', type: 'Contact Us', question: '' }, format: :js
