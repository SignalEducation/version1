...
describe CourseModulesController, type: :controller do
  ...
  let!(:subject_course_1)  { FactoryBot.create(:active_subject_course,
                                               group_id: group_1.id,
                                               exam_body_id: 1) }
  let!(:course_module_1) { FactoryBot.create(:active_course_module, subject_course_id: subject_course_1.id) }
  let!(:course_module_2) { FactoryBot.create(:active_course_module, subject_course_id: subject_course_1.id) }

  let!(:valid_params) { FactoryBot.attributes_for(:course_module, subject_course_id: subject_course_1.id) }
