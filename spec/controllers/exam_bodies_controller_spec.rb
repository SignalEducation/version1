...
describe ExamBodiesController, type: :controller do
  ...
  let!(:level) { FactoryBot.create(:level, group_id: group.id) }
  let!(:course)  { FactoryBot.create(:active_course, level_id: level.id, group_id: group.id, exam_body_id: exam_body_1.id) }
