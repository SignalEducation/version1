...
shared_context 'course_content' do
  ...
  # Available on Trial
  let!(:course_step_1)    { create(:course_step, :cme_quiz, course_lesson: course_lesson_1, available_on_trial: true) }
  let!(:course_quiz) { create(:course_quiz, course_step_id: course_step_1.id) }

  # Available on Trial
  let!(:course_step_2)       { create(:course_step, :cme_video, course_lesson: course_lesson_1, available_on_trial: true) }
  let!(:course_video_1) { create(:course_video, course_step_id: course_step_2.id, vimeo_guid: 'abc123', dacast_id: 1234) }

  # Subscription Required
  let!(:course_step_3)       { create(:course_step, :cme_video, course_lesson: course_lesson_1) }
  let!(:course_video_2) { create(:course_video, course_step_id: course_step_3.id, vimeo_guid: '123abc', dacast_id: 1234) }

  let!(:course_step_4) { create(:course_step, :cme_constructed_response, course_lesson: course_lesson_1, available_on_trial: true) }
