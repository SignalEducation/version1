...
RSpec.describe 'student_user_types/edit', type: :view do
  before(:each) do
    @student_user_type = FactoryGirl.create(:student_user_type)
  end

  it 'renders new student_user_type form' do
    render
    assert_select 'form[action=?][method=?]', student_user_type_path(id: @student_user_type.id), 'post' do
