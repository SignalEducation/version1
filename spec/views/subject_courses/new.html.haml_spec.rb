...
RSpec.describe 'subject_courses/new', type: :view do
  before(:each) do
    x = FactoryGirl.create(:tutor)
    @tutors = Tutor.all
    @subject_course = FactoryGirl.build(:subject_course)
  end

  it 'renders edit subject_course form' do
    render
    assert_select 'form[action=?][method=?]', subject_courses_path, 'post' do
