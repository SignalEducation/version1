...
RSpec.describe 'user_activity_logs/edit', type: :view do
  before(:each) do
    x = FactoryGirl.create(:user)
    @users = User.all
    x = FactoryGirl.create(:session)
    @sessions = Session.all
    @user_activity_log = FactoryGirl.create(:user_activity_log)
  end

  xit 'renders new user_activity_log form' do
    render
    assert_select 'form[action=?][method=?]', user_activity_log_path(id: @user_activity_log.id), 'post' do
