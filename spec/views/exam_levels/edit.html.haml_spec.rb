...
RSpec.describe 'exam_levels/edit', type: :view do
  before(:each) do
    ...
    @tutors = Tutor.all
    @course_module = FactoryGirl.create(:course_module)
  end

  xit 'renders new exam_level form' do
    render
    assert_select 'form[action=?][method=?]', course_module_path(id: @course_module.id), 'post' do
