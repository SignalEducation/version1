...
RSpec.describe 'subscription_plan_categories/new', type: :view do
  before(:each) do
    @subscription_plan_category = FactoryGirl.build(:subscription_plan_category)
  end

  xit 'renders edit subscription_plan_category form' do
    render
    assert_select 'form[action=?][method=?]', subscription_plan_categories_path, 'post' do
