...
RSpec.describe 'mock_exams/new', type: :view do
  before(:each) do
    x = FactoryGirl.create(:subject_course)
    @subject_courses = SubjectCourse.all
    x = FactoryGirl.create(:product)
    @products = Product.all
    @mock_exam = FactoryGirl.build(:mock_exam)
  end

  it 'renders edit mock_exam form' do
    render
    assert_select 'form[action=?][method=?]', mock_exams_path, 'post' do
