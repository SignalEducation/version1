...
class SubscriptionPaymentCard < ApplicationRecord
  ...
  # Constants
  STATUSES = %w[card-live not-live expired].freeze

  # relationships
  has_many :charges
  belongs_to :user
  belongs_to :account_address_country, class_name: 'Country', foreign_key: :account_country_id
  ...
  def self.build_from_stripe_data(stripe_card_data, user_id = nil)
    ...
    iso_code     = stripe_card_data[:country].to_s.upcase
    country_id   = Country.find_by(iso_code: iso_code).try(:id) || user.try(:country_id)
    payment_card = SubscriptionPaymentCard.new(
      user_id: user.id,
    ...
    return true if payment_card.save
  rescue => e
    Rails.logger.error "SubscriptionPaymentCard#build_from_stripe_data - failed to save a new record. Errors: #{e}"
    false
  end
  ...
  def create_on_stripe_using_token
    if stripe_token.to_s.length > 0 && user.stripe_customer_id && stripe_card_guid.blank?
      ...
      if stripe_customer && new_card_hash
        ...
        Rails.logger.error "ERROR: Could not find Stripe Customer with-#{user.stripe_customer_id}. OR Stripe Create card failed with response-#{new_card_hash}."
      end
    else
      Rails.logger.error "ERROR: Could not find Stripe Token-#{stripe_token} OR a customer id in the User-#{user.stripe_customer_id}"
    end
  ...
  def remove_card_from_stripe
    ...
    if e.response.data[:error][:message] == "No such source: #{stripe_card_guid}"
      true
    else
      Rails.logger.error "ERROR: SubscriptionPaymentCard#delete_card failed. Error: #{e.inspect}. Self: #{errors.inspect}"
      errors.add(:base, I18n.t('models.subscriptions.upgrade_plan.processing_error_at_stripe'))
      false
    end
  end
