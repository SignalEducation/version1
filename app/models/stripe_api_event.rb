...
class StripeApiEvent < ActiveRecord::Base
  ...
  def get_data_from_stripe
    if self.guid
      ...
      self.payload = JSON.parse(response[:event], {symbolize_names: true})
      true
    else
      errors.add(:payload, I18n.t('models.stripe_api_event.guid_required_to_get_payoad_from_stripe'))
      false
    end
  end
  ...
  def disseminate_payload
    if self.payload && self.payload.class == Hash && self.payload[:type]
      payload_data_object = payload[:data][:object]
      ...
      case self.payload[:type]
        ...
        when 'invoice.created'
          item_saved = Invoice.create_from_stripe_event(payload_data_object)
      ...
      if item_saved[:response] == true
        self.processed = true
      else
        self.processed = false
        self.error_message = item_saved[:error]
        self.error = true
      end
      true
    else
