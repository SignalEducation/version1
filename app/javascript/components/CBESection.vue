    ...
    <div>
      ...
      <div v-show="this.$emit.showQuestions">
        <span class="badge badge-pill badge-primary">
          Section ID {{this.$store.state.currentSectionId}}
        </span>
      </div>
    </div>
  </div>
  ...
  export default {
    ...
    methods: {
      saveSection: function(page, index) {
        ...
        axios
          .post("http://localhost:3000/api/v1/cbe/sections",{ cbe_section: this.sectionDetails })
          .then(response => {
            this.createdSection = response.data;
            this.$store.commit(
              "setCurrentSectionId",
              this.createdSection.cbeSectionId
            );
