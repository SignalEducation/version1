  ...
  methods: {
		saveNewMultipleChoiceQuestion: function (page, index) {
			...
			axios.post('http://localhost:3000/api/cbe_multiple_choice_questions/', {cbe_multiple_choice_question: this.multipleChoiceDetails} )
				.then(response => {
					console.log("From backend --> " +response.data.cbeMultipleChoiceQuestionId)
					this.$store.commit('currentMultilpleQuestionId', response.data.cbeMultipleChoiceQuestionId)
				 	this.returnedQId = this.$store.state.currentMultilpleQuestionId
					console.log("LOG TEST >>> " + this.returnedQId)
