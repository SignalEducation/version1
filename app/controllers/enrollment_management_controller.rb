...
class EnrollmentManagementController < ApplicationController
  ...
  def export_log_data
    ...
    @course_step_logs = @scul.course_step_logs

    respond_to do |format|
      format.html
      format.csv { send_data @course_step_logs.to_csv }
      format.xls { send_data @course_step_logs.to_csv(col_sep: "\t", headers: true), filename: "enrolment-#{@enrollment.id}-#{Date.today}.xls" }
    end
  end
