...
module CbeQuestionsHelper
  ...
  def question_corrected_link(question)
    ...
    tag.div class: 'question-listed' do
      if cbe_question.kind == 'open' || cbe_question.kind == 'spreadsheet'
        button_tag "*#{cbe_question.kind.humanize}",
                   'class': 'btn btn-link', 'aria-controls': "collapse_#{question.id}",
                   'aria-expanded': 'false', 'data-toggle': 'collapse',
                   "data-target": "#collapse_#{question.id}"
      else
        cbe_question.kind.humanize
      end
    end
  end
